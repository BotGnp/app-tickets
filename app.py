...
def api_tickets_pendientes():
    cur = mysql.connection.cursor()
    cur.execute("""
        SELECT id, ticket, nombre_cliente, telefono, 
            DATE_FORMAT(fecha_creacion, '%%Y-%%m-%%d %%H:%%i:%%s') as fecha_creacion
        FROM registros_ot
...
@login_required
@role_required(['admin'])
def vista_pendientes_admin():
    return render_template('pendientes_admin.html')
...
def ver_ticket(numero_ot):
    ...
    if request.method == 'POST':
    # ❌ Bloquear acciones si el ticket ya está cerrado
        if ticket['estado'] == 'cerrado':
            flash('Este ticket está cerrado y no se pueden realizar más acciones.', 'warning')
    ...
    if accion == 'actualizar_estado':
        ...
        if ticket['bloqueado_por'] != current_user.id:
            ...
            # ✅ Borrar historial del chat si el ticket se cierra
            chat_id_to_use = ticket['chat_id'] if ticket['chat_id'] else str(ticket['id'])
            if nuevo_estado == 'cerrado' and estado_anterior != 'cerrado':
                cur.execute("DELETE FROM mensajes_tecnicos WHERE chat_id = %s", (chat_id_to_use,))
                mysql.connection.commit()
                cur.execute("UPDATE registros_ot SET fecha_finalizacion = NOW() WHERE ticket = %s", (numero_ot,))
                mysql.connection.commit()
            ...
            if nuevo_estado != 'cerrado' and chat_id_to_use:
        ...
        if ticket['bloqueado_por']:
            flash('Este ticket ya ha sido tomado por otro usuario.', 'danger')
        else:
            cur.execute("""
            UPDATE registros_ot 
        ...
        mysql.connection.commit()
        registrar_historial(cur, ticket['id'], 'asignacion', f'Ticket tomado por {current_user.nombre}')
        flash('Has tomado el control del ticket.', 'success')
        return redirect(url_for('ver_ticket', numero_ot=numero_ot))
...
def send_telegram_message(chat_id, message, ticket_id=None):
    try:
        if ticket_id:
            cur = mysql.connection.cursor()
            cur.execute("SELECT estado FROM registros_ot WHERE id = %s", (ticket_id,))
            ticket = cur.fetchone()
            cur.close()
            if ticket and ticket['estado'] == 'cerrado':
                return None  # No enviar mensaje
        response = requests.post(f'{TELEGRAM_API_URL}/sendMessage', data={
            'chat_id': chat_id,
            'text': message,
            'parse_mode': 'Markdown'
        })
...
def reportes():
    ...
    # Total de tickets
    cur.execute("SELECT COUNT(*) as total FROM registros_ot")
    total_tickets = cur.fetchone()['total']
    # Total cerrados
    cur.execute("SELECT COUNT(*) as total FROM registros_ot WHERE estado = 'cerrado'")
    cerrados = cur.fetchone()['total']
    # Usuario más demorado
    cur.execute("""
    SELECT u.nombre, AVG(TIMESTAMPDIFF(MINUTE, r.fecha_creacion, r.fecha_finalizacion)) as promedio_min
    FROM registros_ot r
    JOIN usuarios u ON r.bloqueado_por = u.id
    WHERE r.estado = 'cerrado' AND r.fecha_finalizacion IS NOT NULL
    GROUP BY u.id
    ...
    LIMIT 1
    """)
    usuario_mas_lento = cur.fetchone()
    # Usuario que más cierra
    cur.execute("""
    SELECT u.nombre, COUNT(*) as cerrados
    FROM registros_ot r
    JOIN usuarios u ON r.bloqueado_por = u.id
    ...
    WHERE r.estado = 'cerrado'
    GROUP BY u.id
    ORDER BY cerrados ASC
    LIMIT 1
    """)
    usuario_menos_activo = cur.fetchone()
    # Detalle por ticket cerrado
    cur.execute("""
    SELECT r.ticket, u.nombre as asesor,
        r.fecha_creacion, r.fecha_finalizacion,
        TIMESTAMPDIFF(MINUTE, r.fecha_creacion, r.fecha_finalizacion) as duracion_min
    FROM registros_ot r
    JOIN usuarios u ON r.bloqueado_por = u.id
    WHERE r.estado = 'cerrado' AND r.fecha_finalizacion IS NOT NULL
    ORDER BY r.fecha_finalizacion DESC
    """)
    detalle_tickets = cur.fetchall()
    cur.close()
    ...
    return render_template(
        ...
        cerrados=cerrados,
        usuario_mas_lento=usuario_mas_lento,
        usuario_mas_activo=usuario_mas_activo,
        usuario_menos_activo=usuario_menos_activo,
